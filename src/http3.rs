...
pub async fn serve_h3(
    ...
    let (endpoint, mut incoming) = Endpoint::server(server_config, address.clone())?;
...
async fn handle_h3_request(
    ...
    *hyper_req.version_mut() = parts.version;
    ...
    let body_str = body;
    let body_bytes = Bytes::from(body_str.as_bytes());

    let mut resp = h3::server::Response::new(());
    ...
    *resp.headers_mut() = parts.headers;
    ...
    if !body_bytes.is_empty() {
        stream.send_data(Full::new(body_bytes)).await?;
    }

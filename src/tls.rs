...
pub fn load_certs(filename: &str) -> Result<Vec<pki_types::CertificateDer<'static>>, Box<dyn std::error::Error>> {
...
pub fn load_private_key(filename: &str) -> Result<pki_types::PrivateKeyDer<'static>, Box<dyn std::error::Error>> {
    let contents = std::fs::read(filename)?;

    // Try PKCS8 first (most common)
    for key in rustls_pki_types::pem::SliceIter::<pki_types::PrivatePkcs8KeyDer>::new(&contents) {
        return Ok(pki_types::PrivateKeyDer::Pkcs8(key?));
    }

    // Try SEC1 (EC keys)
    for key in rustls_pki_types::pem::SliceIter::<pki_types::PrivateSec1KeyDer>::new(&contents) {
        return Ok(pki_types::PrivateKeyDer::Sec1(key?));
    }

    // Try PKCS1 (RSA keys)
    for key in rustls_pki_types::pem::SliceIter::<pki_types::PrivatePkcs1KeyDer>::new(&contents) {
...
pub fn create_tls_config(
    ...
    let mut config = if tls_config.require_client_certs {
        let ca_cert = tls_config.ca_cert.as_ref().ok_or_else(|| {
...
pub fn create_quic_server_config(
    tls_config: &TlsConfig,
) -> Result<QuinnServerConfig, Box<dyn std::error::Error>> {
    let certs = load_certs(&tls_config.server_cert)?;
    let key = load_private_key(&tls_config.server_key)?;
